...
data_olahan <- data_olah[,-c(1,3,4,5,6)]
```

```{r}
# Cek Korelasi
for (i in 1:ncol(data_olahan)){
  a <- cor.test(x=as.matrix(data_olahan[i]), y=data_olahan$APS_Disab, method="pearson" )
}
...
for (i in 1:(ncol(data_olahan)-1)) {
  ...
  a <- cor.test(x = data_olahan[[i]], y = data_olahan$APS_Nondis, method = "pearson")
  
  # Menentukan keputusan berdasarkan p-value
  cutoff <- ifelse(a$estimate >= 0.5, "Korelasi >= 0,5", 
                     ifelse(a$estimate >= 0.3,"Korelasi >= 0,3",
                            ifelse(a$estimate >= 0.2,"Korelasi >= 0,2",
                                   ifelse(a$estimate >= 0.1,"Korelasi >= 0,1","Korelasi Rendah"))))
