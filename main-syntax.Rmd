...
```{r}
# Membuat data frame kosong
cor <- data.frame(Variable = character(),
                      Correlation = numeric(),
                      Cutoff = character(),
                      P_Value = numeric(),
                      Decision = character(),
...
for (i in 1:(ncol(data_olahan))) {
  ...
  cor <- rbind(cor, data.frame(Variable = names(data_olahan)[i],
                                       Correlation = a$estimate,
                                       Cutoff = as.factor(cutoff),
                                       P_Value = a$p.value,
                                       Decision = as.factor(decision)))
...
# Menggunakan Cutoff Korelasi >= 0.2
HB6 <- saeHB::Beta(formula = APS_Disab ~ 
                     Rasio_murid_smp +
                     keluarga_listrik +
                     Persen_desa_luar_hutan +
                     Rasio_nakes_faskes +
...
# Cek RSE
summary(
  as.data.frame(HB6$Est$SD/HB6$Est$MEAN*100) %>%
  mutate(
    RSE = as.factor(ifelse(HB6$Est$SD/HB6$Est$MEAN*100 >= 50, ">=50%",
                           ifelse(HB6$Est$SD/HB6$Est$MEAN*100 >= 25, ">=25%",
                                  "<25%")))
    )
)
